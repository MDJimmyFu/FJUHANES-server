...
# Search for Outpatient case tables
print("[*] Searching for PAT/OPD combinations...")
q = "SELECT OWNER, TABLE_NAME FROM ALL_TABLES WHERE (TABLE_NAME LIKE 'PAT%OPD%' OR TABLE_NAME LIKE 'OPD%PAT%' OR TABLE_NAME LIKE '%VISIT%') AND ROWNUM <= 50"
res = execute_sql(q)
rows = parse_rows(res)
for r in rows:
