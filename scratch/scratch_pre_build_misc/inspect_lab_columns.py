...
class ColumnInspector(HISClient):
...
hunter = ColumnInspector()

tables = ['LABIHEM', 'LABIHED', 'EXMRPTM', 'EXMREPD']

for t in tables:
    print(f"\nColumns for OPDUSR.{t}:")
    sql = f"SELECT COLUMN_NAME FROM ALL_TAB_COLUMNS WHERE OWNER='OPDUSR' AND TABLE_NAME='{t}' ORDER BY COLUMN_ID"
    res = hunter.execute_sql(sql)
    
    if res and "<NewDataSet>" in res:
        matches = re.findall(r'<COLUMN_NAME>(.*?)</COLUMN_NAME>', res)
        print(matches)
